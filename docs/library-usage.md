...
from compere.modules.entity import create_entity, get_entities
...
```python
from compere.modules.entity import get_entities

# Get all entities
entities = get_entities(db, skip=0, limit=100)

# Search by name
results = get_entities(db, search="Product")
```
...
```python
from compere.modules.comparison import get_comparisons

# All comparisons
comparisons = get_comparisons(db, skip=0, limit=100)

# Filter by entity
entity_comparisons = get_comparisons(db, entity_id=1)
```
...
# Manually update ratings (usually handled by create_comparison)
update_elo_ratings(winner_id=1, loser_id=2, db=db)
```
...
result = get_next_comparison(db)

if result:
    entity1, entity2 = result["entity1"], result["entity2"]
    print(f"Compare: {entity1.name} vs {entity2.name}")
```

### UCB Algorithm Details
...
pull_counts = {1: 10, 2: 5, 3: 2, 4: 0, 5: 8}
rewards = {1: 7, 2: 3, 3: 1, 4: 0, 5: 6}

# Select next entity to compare
selected_id = ucb.select(entity_ids, pull_counts, rewards, total_pulls=25)
```
...
for name, desc in products:
    create_entity(EntityCreate(name=name, description=desc), db)
...
next_pair = get_next_comparison(db)
if next_pair:
...
try:
    entity = create_entity(EntityCreate(name="Test"), db)
